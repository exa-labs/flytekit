...
class RayJobConfig:
    """
    Configuration for Ray tasks.

    Supports two modes:
    1. Create a new RayCluster (default): Provide worker_node_config and optionally head_node_config
    2. Use existing RayCluster: Provide cluster_selector labels to match an existing cluster

    When cluster_selector is provided, the task submits to an existing RayCluster instead of
    creating a new one. This enables long-lived clusters for faster iteration during development.

    Example using existing cluster:
        ray_config = RayJobConfig(
            cluster_selector={"ray.io/cluster": "my-dev-cluster"},
            runtime_env={"pip": ["numpy"]},
        )

    Example creating new cluster:
        ray_config = RayJobConfig(
            worker_node_config=[WorkerNodeConfig(group_name="workers", replicas=2)],
            head_node_config=HeadNodeConfig(requests=Resources(cpu="2", mem="4Gi")),
        )
    """
    worker_node_config: typing.Optional[typing.List[WorkerNodeConfig]] = None
    ...
    def __post_init__(self):
        if self.cluster_selector is not None and self.worker_node_config is not None:
            raise ValueError("cluster_selector and worker_node_config are mutually exclusive")
        if self.cluster_selector is None and self.worker_node_config is None:
            raise ValueError("Either cluster_selector or worker_node_config must be provided")
...
class RayFunctionTask(PythonFunctionTask):
    ...
    def get_custom(self, settings: SerializationSettings) -> Optional[Dict[str, Any]]:
        ...
        if cfg.cluster_selector is not None:
            ray_job = RayJob(
                cluster_selector=cfg.cluster_selector,
